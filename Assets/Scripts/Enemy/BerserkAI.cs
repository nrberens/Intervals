...
public class BerserkAI : FSM {
    ...
    protected void UpdateSeekLOSState() {
        ...
        int randomCoin = UnityEngine.Random.Range(0, 2);
        if (randomCoin == 1) {
            shootDir = GetDirectionToNode(_ec.Mover.currentNode, PlayerController.pc.Mover.currentNode);

            if (shootDir != null) {
                Direction dir = (Direction)shootDir;
                StartCoroutine(_ec.Shooter.Shoot(dir));
                return;
            }
        }
